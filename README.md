...
│   ├── db_export.py              # loads CSV outputs into a SQLite database
│   └── run_queries.py            # example SQL queries against the database
...
flowchart LR
    ...
    CSV1 --> DB["db_export.py -> tp_pipeline.db"]
    CSV2 --> DB
    CSV3 --> DB
    DB --> QUERIES["run_queries.py"]
```
...
Beyond the CSV outputs, the pipeline results can be loaded into a SQLite
database for SQL-based analysis:

```bash
python3 src/tp_pipeline/db_export.py    # rebuild output/tp_pipeline.db (full refresh)
python3 src/tp_pipeline/run_queries.py  # run example queries against it
```

`run_queries.py` includes filtering, aggregation, joins, and a window function ranking distributors by IC volume
within their region.
